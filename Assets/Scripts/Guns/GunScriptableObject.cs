    ...
    public class GunScriptableObject : ScriptableObject
    {
        ...
        private void Shoot()
        {
            ...
            if (Time.time > shootConfig.fireRate + _lastShootTime)
            {
                ...
                Ray screenCenterRay = Camera.main.ScreenPointToRay(screenCenter);
                // Perform a raycast from the camera to the center of the screen to get the hit point
                RaycastHit screenCenterHit;
                Vector3 shootDirection;

                if (Physics.Raycast(screenCenterRay, out screenCenterHit, float.MaxValue, shootConfig.hitMask))
                {
                    // If the ray from the screen center hits something, calculate the direction from the gun to the hit point
                    shootDirection = (screenCenterHit.point - _shootSystem.transform.position).normalized;
                }
                else
                {
                    // If the ray doesn't hit anything, use the ray direction directly
                    shootDirection = screenCenterRay.direction;
                }
                shootDirection += _model.transform.TransformDirection(spreadAmount);
                OnShoot?.Invoke();
